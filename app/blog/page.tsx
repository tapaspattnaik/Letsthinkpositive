...
const posts = [
  { tag: 'Mindset',      author: 'Tapas', title: 'The Day I Stopped Looking for Superman',                              excerpt: 'As a kid, I wanted to be saved by a superhero. As an adult, I realised the world needs people who will truly listen.' },
  { tag: 'Wellness',     author: 'Tapas', title: 'What Gandhi\'s 3 Monkeys Taught Me About Modern Life',                excerpt: 'I grew up with those three monkeys on the wall. Then life happened — and I understood it completely differently.' },
  { tag: 'Student Life', author: 'Tapas', title: 'You Are Not Behind. You Are Exactly Where You Need to Be.',           excerpt: 'Somewhere a student is comparing their chapter 1 to someone else\'s chapter 20. This post is for them.' },
  { tag: 'Career',       author: 'Tapas', title: 'They Can Take Your Job. But Not This.',                               excerpt: 'The difference between those who rebuild and those who fall apart was never the circumstances. It was always the thinking.' },
  { tag: 'Habits',       author: 'Tapas', title: 'I Changed One Thing Every Morning. Six Months Later, Everything Was Different.', excerpt: 'Not 5am wake-ups. Not cold showers. Just three minutes that quietly rewired how I face every day.' },
]

const filters = ['All', 'Mindset', 'Wellness', 'Career', 'Student Life', 'Habits', 'Yoga 🧘', 'Positive Eating 🥗']
            ...
            {filters.map((f, i) => (
              <button key={f}
                className={`px-5 py-2 rounded-full border text-[0.85rem] font-medium transition-all
                  ${i === 0 ? 'bg-teal-deep text-white border-teal-deep' : 'bg-white text-text-mid border-teal-light hover:bg-teal-deep hover:text-white hover:border-teal-deep'}`}>
                {f}
              </button>
            ))}
          </div>
      ...
      <section className="py-16 px-[5%]">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="bg-gradient-to-r from-teal-deep to-teal-mid rounded-[24px] p-8 flex flex-col sm:flex-row justify-between items-center gap-6 mb-10">
            ...
            <Link href="/contact" className="flex-shrink-0 bg-amber text-charcoal px-6 py-3 rounded-full font-semibold text-[0.92rem] no-underline hover:bg-amber-soft transition-colors whitespace-nowrap">
              Submit Your Post →
            </Link>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {posts.map(p => (
              <div key={p.title} className="bg-white rounded-[24px] overflow-hidden shadow-card border border-teal-light/30 hover:-translate-y-1.5 hover:shadow-lift transition-all duration-300">
                ...
                <div className="px-7 py-4 border-t border-teal-ghost flex justify-between items-center">
                  <a href="#" className="text-[0.82rem] font-semibold text-teal-mid no-underline hover:text-teal-deep transition-colors">Read more →</a>
                  <span className="text-[0.76rem] text-text-xlight">By {p.author}</span>
                </div>
              </div>
            ))}
            ...
            <div className="bg-teal-ghost rounded-[24px] overflow-hidden border-2 border-dashed border-teal-light">
              ...
              <div className="px-7 py-4 border-t border-teal-light/50 flex justify-center">
                <Link href="/contact" className="text-[0.82rem] font-semibold text-teal-mid no-underline hover:text-teal-deep transition-colors">Submit your post →</Link>
              </div>
            </div>
          </div>
        </div>
