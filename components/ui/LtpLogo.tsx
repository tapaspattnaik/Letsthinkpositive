...
export function LtpLogo({ size = 54 }: { size?: number }) {
  // useId ensures each instance gets a unique gradient id — prevents rendering
  // issues when the logo appears more than once on the same page (e.g. navbar + drawer)
  const uid = useId().replace(/:/g, '')
  const gradId = `ltpGrad-${uid}`
  ...
  return (
    <svg
      ...
      aria-label="letsthinkpositive logo"
      aria-hidden="true"
    >
      ...
      <circle cx="27" cy="27" r="11.5" fill={`url(#${gradId})`} />
